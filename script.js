...
async function getTracksAudioFeatures(something) {
  const response = await fetch(`https://api.spotify.com/v1/audio-features?ids=${something}`, {
    method: "GET",
    headers: { Authorization: "Bearer " + currentToken.access_token },
  });
...
const pointBackgroundColor = {
  'sadboi': 'rgb(18,22,25)',
  'angryboi': 'rgb(255,54,171)',
  'chillboi': 'rgb(136,209,138)',
  'hyperboi': 'rgb(1,186,239)'
}
...
form.addEventListener("submit", async (submitEvent) => {
  ...
  const processedData = processChartData(tracks, audioFeatureResponse.audio_features);
